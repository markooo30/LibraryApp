    ...
    class Library
    {
        ...
        public void LoadData()
        {
            if (File.Exists("books.txt"))
                books = DataHandler.LoadBooksFromFile("books.txt");

            if (File.Exists("users.txt"))
                users = DataHandler.LoadUsersFromFile("users.txt");

            if (File.Exists("transactions.txt"))
                transactions = DataHandler.LoadTransactionsFromFile("transactions.txt");
        }
            ...
            while (true)
            {
                ...
                switch (choice)
                {
                    ...
                    case 4:
                        ...
                        DataHandler.SaveBooksToFile(books, "books.txt");
                        DataHandler.SaveUsersToFile(users, "users.txt");
                        DataHandler.SaveTransactionsToFile(transactions, "transactions.txt");
        ...
        class DataHandler 
        {
            ...
            {
                if (!File.Exists(fileName)) return new List<Book>();
